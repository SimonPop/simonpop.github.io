(function () {
	...
	function draw() {
		...
		// Side by side on wide screens; stacked (matrix over graph) on narrow ones,
		// so the coordinate space itself shrinks rather than getting uniformly
		// scaled down to fit, which is what kept text legible.
		const width = isNarrow ? 320 : 640;
		const height = isNarrow ? 480 : 310;
		const mLeft = isNarrow ? (width - nodesOrder.length * cell) / 2 : 60;
		const mTop = isNarrow ? 30 : 40;
		const NODE_R = 15;
		const R = isNarrow ? 85 : 110;
		const gCenterX = isNarrow ? width / 2 : 430;
		const gCenterY = isNarrow ? mTop + nodesOrder.length * cell + 50 + R : 160;
